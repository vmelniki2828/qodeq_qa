      ...
      {isCreateModalOpen && (
        <ModalOverlay onClick={handleCloseCreateModal}>
          <ModalContent theme={theme} $isCreateModal={true} onClick={(e) => e.stopPropagation()}>
            ...
            <FormSection>
              <FormLabel theme={theme}>Agents</FormLabel>
              <AgentsInput>
                {createForm.agents.map((agent, index) => (
                  <AgentInputRow key={index}>
                    <FormInput
                      theme={theme}
                      type="text"
                      value={agent}
                      onChange={(e) => handleAgentChange(index, e.target.value)}
                      placeholder="Введите UUID агента"
                      style={{ flex: 1 }}
                    />
                    {createForm.agents.length > 1 && (
                      <RemoveAgentButton
                        theme={theme}
                ...
                <AddAgentButton theme={theme} onClick={handleAddAgent}>
                  + Добавить агента
                </AddAgentButton>
              </AgentsInput>
            </FormSection>
