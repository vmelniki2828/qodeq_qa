...
export const ProjectsPage = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        ...
        const url = `/api/v1/settings/project/?${params.toString()}`;
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/settings/project/${editingProject.id}`, {
        method: 'PATCH',
        headers,
  ...
  const handleDelete = async (project, e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/settings/project/${project.id}`, {
        method: 'DELETE',
        headers
      });
  ...
  const handleSaveCreate = async () => {
    try {
      ...
      const res = await fetch('/api/v1/settings/project/', {
        method: 'POST',
        headers,
        body: JSON.stringify(requestBody)
      });
